...
- **v0.4.5 (now): coverage backfill. Four UI smoke tests via Streamlit's `AppTest` runner (would have caught the matter-creation crash and the SQLite cross-thread error before users hit them) and one real-LLM end-to-end regression test against the Anthropic API (gated on `ANTHROPIC_API_KEY` so it only runs when you ask for it).**
